(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('activeBeanFitnessApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-activeBeanFitnessApp-error');
                var entityKey = httpResponse.headers('X-activeBeanFitnessApp-params');
                if (errorHeader) {
